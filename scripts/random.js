...
Random._smoothNoise = function(baseNoise, interpolation, octave)
{
 ...
 */
Random.generateRandomNoise = function(width, height)
{
 ...
 */
Random.generatePerlinNoise = function(width, height, octaveCount, persistance = .5, interpolation = "Linear", distribution = "Default")
{
	let perlinNoise;
	if (octaveCount > 1)
	{
		const baseNoise = Random.generateRandomNoise(width, height);
		let amplitude = 1.;
		...
		{
			const smoothedNoise = Random._smoothNoise(baseNoise, interpolation, octave);
	...
	{
		perlinNoise = Random.generateRandomNoise(width, height);
	}
	// Postprocessing:
	...
	else if (distribution == 'Uniform')  // Horizontal curve with no peak
	{
		...
		for (i = 0; i < Math.trunc(width / 2); i++)
		{
			for (j = 0; j < height; j++)
			{
