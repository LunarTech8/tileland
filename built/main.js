    ...
    constructor(cols, rows) {
        ...
        for (let c = 1; c < this._cols - 1; c++) {
            for (let r = 1; r < this._rows - 1; r++) {
                ...
                if (t.height > 0) {
                    let hasCliff = false;
                    if (Math.abs(t.height - this.tiles.get(c - 1, r).height) >= Main.settings.cliffMinDiff) {
                        hasCliff = true;
                    }
                    else if (Math.abs(t.height - this.tiles.get(c + 1, r).height) >= Main.settings.cliffMinDiff) {
                        hasCliff = true;
                    }
                    else if (Math.abs(t.height - this.tiles.get(c, r - 1).height) >= Main.settings.cliffMinDiff) {
                        hasCliff = true;
                    }
                    else if (Math.abs(t.height - this.tiles.get(c, r + 1).height) >= Main.settings.cliffMinDiff) {
                        hasCliff = true;
...
Game.init = function () {
    ...
    Main.map = new TileMap(Main.settings.mapTilesX, Main.settings.mapTilesY);
...
Game.update = function (delta) {
    ...
    if (Keyboard.isKeyDown(Keyboard.SHIFT)) {
        Main.camera.speed = Main.CAMERA_SPEED_FAST;
    }
    else {
        Main.camera.speed = Main.CAMERA_SPEED_NORMAL;
    }
