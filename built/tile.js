...
// Data code
// --------------------
Tile.TILE_SIZE_TERRAIN = 64;
Tile.TILE_SIZE_VEGETATION = 64;
Tile.Climate = Object.freeze({
    ICY: 1,
    COLD: 2,
    MILD: 3,
    WARM: 4,
...
Tile._determineTileClimate = function (latitudeFactor) {
    latitudeFactor = Utilities.clip(latitudeFactor + 0.05 * (Math.random() - 0.5), 0, 1);
    if (latitudeFactor >= 0.45 && latitudeFactor <= 0.55) {
        return Tile.Climate.HOT;
    }
    else if (latitudeFactor >= 0.35 && latitudeFactor <= 0.65) {
        return Tile.Climate.WARM;
    }
    else if (latitudeFactor >= 0.15 && latitudeFactor <= 0.85) {
        return Tile.Climate.MILD;
    }
    else if (latitudeFactor >= 0.05 && latitudeFactor <= 0.95) {
        return Tile.Climate.COLD;
    }
    else if (latitudeFactor >= 0.00 && latitudeFactor <= 1.00) {
        return Tile.Climate.ICY;
    }
    else {
        throw new Error('Invalid given latitudeFactor (' + latitudeFactor + '), has to be between 0 and 1');
    }
};
Tile._determineFertility = function (climate, height, fertilityNoise) {
    switch (climate) {
        case Tile.Climate.ICY:
            if (height <= 500) {
                return fertilityNoise;
            }
            else {
                return 0;
            }
...
Tile._determineTerrain = function (height, climate) {
    ...
    if (height <= 0) {
        if (climate == Tile.Climate.ICY) {
            return Tile.Terrain.ICE;
        }
        else {
        ...
        if (climate == Tile.Climate.ICY || climate == Tile.Climate.HOT) {
            return Tile.Terrain.EARTH;
        }
        else {
            return Tile.Terrain.SWAMP;
        }
    }
    else if (height <= 1500) {
        if (climate == Tile.Climate.ICY) {
            return Tile.Terrain.SNOW;
        }
        else if (climate == Tile.Climate.HOT) {
            return Tile.Terrain.SAND;
        }
        else {
            return Tile.Terrain.EARTH;
        }
...
Tile._determineVegetation = function (height, climate, fertility) {
    ...
    if (height <= 0) {
        ...
        if (fertility >= 0.8) {
            return Tile.Vegetation.TREES;
        }
        else if (fertility >= 0.7) {
            return Tile.Vegetation.SHRUBS;
        }
        else if (fertility >= 0.4) {
            return Tile.Vegetation.GRASS;
        }
        else if (fertility >= 0.2) {
            return Tile.Vegetation.BARE;
        }
        else if (fertility >= 0.1) {
            return Tile.Vegetation.GRAVEL;
        }
        else {
            return Tile.Vegetation.ROCKS;
        }
    }
    else if (height <= 1500) {
        if (climate == Tile.Climate.ICY || climate == Tile.Climate.HOT) {
...
Tile._getTerrainImageData = function (terrain) {
    switch (terrain) {
...
Tile._getVegetationImageData = function (vegetation) {
    switch (vegetation) {
