...
function smoothNoise(baseNoise: Array2D<number>, interpolation: string, octave: number): Array2D<number>
{
	...
	const smoothNoise = new Array2D<number>(width, height);
	...
	for (let i = 0; i < width; i++)
	{
		...
		for (let j = 0; j < height; j++)
		{
			...
			const top = MUtilities.interpolateLinear(baseNoise.get(horizontalSampleA, verticalSampleA), baseNoise.get(horizontalSampleB, verticalSampleA), horizontalBlend);
			// Blend the bottom two corners:
			const bottom = MUtilities.interpolateLinear(baseNoise.get(horizontalSampleA, verticalSampleB), baseNoise.get(horizontalSampleB, verticalSampleB), horizontalBlend);
			// Final blend:
			smoothNoise.set(i, j, MUtilities.interpolateLinear(top, bottom, verticalBlend));
		}
	}
	return smoothNoise;
}
...
function generateRandomNoise(width: number, height: number): Array2D<number>
{
	const baseNoise = new Array2D<number>(width, height);
	...
	else if (distribution == 'Uniform')  // Horizontal curve with no peak
	{
		...
		for (let i = 0; i < Math.trunc(width / 2); i++)
		{
			for (let j = 0; j < height; j++)
			{
				...
				const acosInput = z1 / MUtilities.divByZeroProt(z1 * z1 + z2 * z2);
